...
| 装扮菜单 | **表情与装扮合并成一个菜单**：44 个表情全部归入 14 个互斥槽位（装扮 7 + 表情 7），每槽单选，**跨槽位可同时生效**。选择**常驻**，由「归位」清空 |
...
44 个表情全部归入 14 个互斥槽位。分类不是按文件名猜的，依据是**模型作者在 cdi3 里自己写的分组和中文名**——例如 `ParamGroup29` 被作者命名为「C款动作开关（集中放置方便检查」，
