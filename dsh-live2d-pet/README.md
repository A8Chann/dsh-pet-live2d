...
> **仍未解决**：装扮槽位还不能同时生效。引擎的表情管理器一次只持有**一个**表情（`currentExpression`），所以选了眼镜再选贴纸，眼镜会被顶掉。槽位数据、判定和文档都在（见 `examples/ds-whale-girl/README.md`），缺的是「多个表情同时渲染」那一块——把参数直接写进 core model 试过（对这套开关参数不生效），把合并结果注册成 blob URL 自定义表达式也试过（没有结论，代码已撤回）。
