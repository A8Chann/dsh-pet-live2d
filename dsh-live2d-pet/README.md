...
点她身上会有反应，判定用的是**模型自己的几何**，不是画出来的区域：

| 互动 | 怎么触发 | 默认反应 |
|---|---|---|
| 摸头 | 点在**头部**（含头发、耳朵 —— 宿主读 cdi3 里作者的部件命名认出来的 21 个部件） | 重锤出击 / 问号 / 星星眼 随机一个，**不脸红** |
| 摸尾巴 | 点在**尾巴/翅膀**上（狐狸尾 / 猫尾 / 狼尾 / 大翅膀…15 个部件） | 吐魂 / 问号 随机一个 |
| 转圈转晕 | 鼠标**绕着角色转圈**：默认 2 圈、1.6 秒内 | 演「晕晕」 |

- 摸头/摸尾巴按**三角面**判定：圆顶带耳朵尖的头部轮廓、尾巴的边缘都对得上，不是方框。
  顶点是**当前帧变形后**的，所以头歪着、身体摆着也准。
- **摸头优先于摸尾巴**：这两个区域可能有重叠（尾巴/翅膀挂件在身后，几何上会伸到头部
  附近），重叠处算摸头。
- **尾巴/翅膀是可选配件**：没戴的时候它们的几何是退化的，"摸尾巴"就是没反应 ——
  这是对的，不是坏了。戴上翅膀/尾巴（装扮槽）之后才摸得到。
- 转圈看的是**累计转角**（不是位置绕了几圈），所以贴着角色转小圈、远远转大圈都算。
- 三个互动各自可开关，反应也能改（见下）。共用的三句台词在「气泡」设置里。

### 气泡：说什么、在哪、要不要

她说话时会弹一个小气泡（问候、点击、摸头、摸尾巴、转晕、归位，以及**每个会话相位一句**：
思考中 / 用工具 / 等你批准 / 等你回答 / 叫了帮手 / 消息排队 / 完成 / 出错）。

- **所有文本都能改**：设置页「气泡」卡里逐条编辑。一组台词**用 `|` 分隔多个变体**
  （随机挑一句）；**留空 = 用宠物默认**（`pet.json` 的 `live2d.lines`），所以"看一眼"不会
  把默认改掉。
- **位置可调**：左右 / 上下偏移两个滑杆（相对她头顶的默认位置），另有"一句话停留多久"。
- **总开关**：关掉之后任何台词都不弹。
