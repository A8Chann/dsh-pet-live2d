...
引擎的表情管理器一次只持有**一个**表情（`expressionManager.currentExpression`），所以「眼镜 + 猫猫贴纸 + 深色桌布 同时戴」不能靠它直接实现。

**已完成**：宿主新增 `/api/live2d-pet/merge?pet=<id>&e=<name>&e=...`，把选中的几个表情合成一份 .exp3.json 返回（参数取并集，同名后者覆盖）。`cdp-merge` 验证了：单槽位参数正确、两个槽位并集包含 `ParamCheek70`+`ParamCheek83`、三个槽位三个参数、未声明的表情名和未知宠物都 404。

**为什么走宿主而不是在浏览器里合成**：引擎按 `new URL(File, modelUrl)` 解析定义的 File，根相对路径会回到本服务；而浏览器里合并需要的 `blob:` URL **不匹配**解析器的绝对 URL 判定，会被改写成相对路径而加载不了——这正是第一版 blob 方案静默失败的原因。

**没完成**：把这个合并结果注册成引擎的一个额外定义**会塌**。淡入确实启动了（实测抓到参数到过 0.02），随后回落到 0；于是「选两个槽位」渲染出**零个**效果——比原来的「后者覆盖」（至少渲染一个）更差。所以客户端那条线**已撤掉**，只保留宿主路由（它是对的，也是后续要用的）。

> **一个关于测量的教训**（写下来免得重蹈）：这一块我前后给出过两个互相矛盾的结论，都是被**错误的测量方法**带偏的。
> - 用「冻结 rAF + 截图逐像素比对」测，每格都是 0.00%，于是判定「表情全都不生效」；实际是冻结方式不可靠，后面每一格都在读同一张陈旧帧。
> - 换成「比对 canvas 哈希」，得到 `changed: true`，于是判定「表情生效了」；实际宠物一直在呼吸眨眼，**任意两帧都不会逐字节相同**，所以这个信号恒为真——拿它当证据等于没测。
>
> 可靠的做法是读**引擎在帧内写进模型的参数值**（`cdp-exp` / `cdp-merge` 现在都这么做）。它是确定性的，不受动画相位影响。结论：**表情是好的**——点墨镜 → `ParamCheek71=1`，点星星眼 → `ParamCheek16=1`，点头顶鲸 → `jingyu=1`，全部实测通过。
