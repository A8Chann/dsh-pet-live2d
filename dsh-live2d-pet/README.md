...
池子里的「默认」是**这次不动**，不是"清空这个槽位"。差别只在槽位当前**不是**默认值时
才看得出来 —— 而那正是用户会碰到的情形：

> "掏出手机后冒爱心的氛围为什么没有了？"

复现出来是这样的：摸鱼每隔一二十秒把每个槽位重掷一次，掷到「默认」就走
`chooseSlotOption(slot, null)` 把槽位**清空**，于是手选的爱心眼被清掉、配对点亮的
冒爱心跟着消失（"掏出手机"纯属巧合 —— 查过 `motion3`，自拍和掏手机都不驱动 `love`）。
现在掷到「默认」直接跳过这个槽位：

- 摸鱼照样在动（手、情绪、嘴这些照常换），只是**不再把已有的东西抹掉**；
- 想让某个槽位回到默认，手动点「无」，或者把「默认」那条从池子里删掉；
- 被「同时」钉住的槽位（爱心眼 → 冒爱心）**不参与抽签**：配对是"一起点亮"，
  摸鱼不该从侧面把它拆掉。

**自拍**现在是独立槽位（`自拍 / 快速自拍`），不再是右手的选项：动作自己就把手机
抬起来（`phone` 曲线 0→1，查过数据），所以原来那条"自拍需要右手=掏出手机"的守卫
去掉了 —— 留着的话选中自拍会被它自己的守卫静默挡掉。
