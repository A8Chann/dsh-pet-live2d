...
> 右键面板里原来还有第三个「设置」页签，是给"改池子不用翻设置"用的**过渡**入口。
> 2.0.0 去掉了：面板只负责"点一下换个样子"，改数值统一去 DSH 设置页那一节
> （两处本来共用同一份值，现在只留一处）。
...
**摸鱼抽签要在"应用的那一刻"再查一遍前提。** 所有池子都是按**这一轮开始前**的状态掷
的，而应用是按槽位顺序逐个进行的 —— 右手掷成「双手比耶」之后，同一轮里被抽中的
「自拍」其实已经没有前提了。用户报的"我设置了拍照的前提是右手手机，为什么还会出现
右手比耶，然后拍照"，就是漏了这一步重查（在手势动作上尤其明显，因为手部槽位排在
自拍槽位前面）。

**选项上的「前提」也会管动作。** 这条原来是断的：动作的前提只查 `pet.json` 的
`motionGuards`，UI 里给选项加的 `requires` 只对**表情**生效 —— 用户给「自拍」加了
「前提：右手 = 掏出手机」，动作那边根本没人看。现在 `motionRequiresFor(group)` 把
"所有 motion 指向这个组的选项"的 requires 汇总起来喂给动作守卫。

两种前提的差别（决定了能不能自动满足）：

| 写法 | 形态 | 能不能替用户补上 |
|---|---|---|
| `pet.json` 的 `motionGuards` | 槽位 = **标签白名单** | 不能，只能拦（喷水要鲸鱼：鲸鱼有好几种） |
| 选项上的 `requires` | 槽位 = **某一个标签** | 能 —— 手动点选时由插件补齐 |

所以：**摸鱼抽签**是闸门（前提不成立就不抽它）；**手动点选**则由插件把前提补上
（点「自拍」会先把手机掏出来，跟点「挤番茄酱」会把蛋包饭端上来是同一套），
否则点了没反应 —— 静默无效正是之前那串 bug 的同一类。
...
设置正文挂在 DSH 设置页那一节里，样式写一份就够了（作用域 `[data-pet-settings]`），
浅色/深色都能用：颜色走 `currentColor` + 中性灰透明度，强调色只出现在带透明度的药丸和
条上（写死颜色必然一边好看一边瞎）。**改外观请改 `SETTINGS_CSS`** —— 行内样式优先级
更高，原来散在组件里的那几处已经全部收进样式表了。

> 面板里那份设置正文（作用域 `[data-panel] [data-settings]`）在 2.0.0 去掉了，
> 但 `SETTINGS_SCOPES` 里那条选择器先留着：它是无害的，而删掉它要动一排 CSS 断言。
>
> 正文组件必须用 `h(PetSettingsBody)` **渲染**，不能写成 `...PetSettingsBody()` 那样
> 直接调用：直接调用会把它的 hooks 算到 `Pet` 头上，React 会抛 "Rendered more hooks
> than during the previous render" 并把**整只宠物**卸载。这个坑真的踩过一次。
