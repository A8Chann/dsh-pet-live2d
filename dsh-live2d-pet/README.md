...
「氛围装饰」（`computeAmbientOnly`）。待机每帧记一份，换成别的动作时写回那一份
（`preserveAmbient`，写在 `saveParameters` 缝里、且在帧内早退**之前**）。

代价：定格期间爱心是**静止**的（保住待机最后那一帧的位置），回到待机才继续飘。
动画数据在 `idle.motion3.json` 的曲线里，引擎没有"逐帧采样某个动作曲线"的接口，
所以"定格时还接着飘"要另做一层程序化动画 —— 目前没做。
