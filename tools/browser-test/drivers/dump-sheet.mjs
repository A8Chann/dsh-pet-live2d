...
const READ = (ids) => ev(`(()=>{const p=window.__PET_DBG.core._model.parameters;const a=Array.from(p.ids);const o={};for(const id of ${JSON.stringify(ids)}){const i=a.indexOf(id);o[id]=i<0?null:Number(p.values[i].toFixed(2))}return JSON.stringify(o)})()`)
const cells = [
  ['none', []],
  ['cat-paws', ['喵喵手']],
  ['hands-peace', ['双手比耶']],
...
for (const [label, names] of cells) {
  ...
  await sleep(1000)
  const params = await READ(['maoshou','phone7','ParamCheek16','ParamCheek17','ParamCheek70','ParamCheek72'])
