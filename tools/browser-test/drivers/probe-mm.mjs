...
const EDGE = browserPath(); const PORT = 9509
const PROFILE = join(PROFILES, '_mm')
...
await sleep(4000)
const q = async (l, e) => console.log(l.padEnd(36) + await ev(e))
await q('motionManager.expressionManager', 'typeof window.__PET_DBG.model.internalModel.motionManager.expressionManager')
await q('mm.settings.expressions len', 'JSON.stringify((window.__PET_DBG.model.internalModel.motionManager.settings?.expressions||[]).length)')
await q('mm expression defs len', 'JSON.stringify((window.__PET_DBG.model.internalModel.motionManager.expressionManager?.definitions||[]).length)')
await q('getExpressionIndex(星星眼)', 'String(window.__PET_DBG.model.internalModel.motionManager.expressionManager?.getExpressionIndex?.("星星眼"))')
const P = '(()=>{const p=window.__PET_DBG.core._model.parameters;const a=Array.from(p.ids);return p.values[a.indexOf("ParamCheek16")]})()'
await q('ParamCheek16 before', P)
await q('await model.expression(星星眼)', 'String(await window.__PET_DBG.model.expression("星星眼"))')
await sleep(1200)
await q('ParamCheek16 after', P)
await q('currentExpression', 'String(!!window.__PET_DBG.model.internalModel.motionManager.expressionManager?.currentExpression)')
