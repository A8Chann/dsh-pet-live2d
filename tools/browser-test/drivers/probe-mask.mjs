...
const EDGE = browserPath(); const PORT = 9467
const PROFILE = join(PROFILES, '_hit2')
...
await send('Page.navigate',{url:'file:///D:/HTML/DSH_Pet_Live2d/tools/browser-test/shots/_hit2.html'})
await sleep(1200)
...
await click(100,100)   // inside the mask circle -> above
await click(10,10)     // masked out -> must reach below
await click(190,190)   // masked out -> must reach below
console.log('mask-image hit targets:', await ev('JSON.stringify(window.hits)'))
console.log('mask-image supported:', await ev("CSS.supports('mask-image','url(x.png)')"))
