...
const PORT = 9443
const PROFILE = join(PROFILES, '_cdp-trace')
...
await sleep(4000)
await ev('window.__PET_TRACE.length = 0')
await ev('window.__dshLive2dPet.playOnce("SelfieQuick",0,{kind:"panel"})')
for (let i = 0; i < 20; i++) {
  await sleep(400)
  console.log(String(i * 400).padStart(5), 'motion=' + (await ev('document.querySelector("[data-dsh-live2d-pet]").getAttribute("data-motion")')), 'trace=' + (await ev('JSON.stringify(window.__PET_TRACE.slice(-3))')))
}
