...
const EDGE = browserPath(); const PORT = 9523
const PROFILE = join(PROFILES, '_layers')
...
await q('layerCount (initial)', 'window.__dshLive2dPet.expressionLayerCount()')
await q('setExpressions ok', 'String(window.__dshLive2dPet.setExpressions(["圆眼镜"]))')
await sleep(800)
await q('pinned', 'JSON.stringify(window.__dshLive2dPet.expressions())')
await q('layerCount (after)', 'window.__dshLive2dPet.expressionLayerCount()')
await q('catalog has params', 'JSON.stringify((window.__dshLive2dPetCatalog||null)===null?"n/a":"n/a")')
