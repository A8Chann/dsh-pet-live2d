...
## 动作语义：hold / persist / prepend / preset

模型自己表达不了"作者意图"，所以 `pet.json` 的 `motionOptions` 补四个开关：

| 键 | 含义 |
|---|---|
| `hold` | 停在最后一帧（定格），而不是播完就交还身体 |
| `persist` | 定格**且不自动放手**（不加它，`ACTION_HOLD_MAX_MS` = 9s 后交还） |
...
- **`loop: false` 必须显式传**。这只模型每个 `motion3.json` 都写着 `"Loop": true`，
  而引擎会把动作自己的 flag 和调用方的合并（`setLoop(loop ?? motionData.loop)`）——
  不传就永远循环、永远停不下来。
- **`prepend` 的前置动作同样受它自己的 `motionOptions` 约束**，而且前置链要保住
  **最初**那份快照，否则"退役"时只撤销最后一段。
...
引擎一次只播一个动作，而"槽位选中的动作选项"可能同时有好几个（右手=掏出手机、
嘴部=吹泡泡糖）。两个决定：

- **谁说了算**：记"最后点的那个槽位"（`motionOwnerRef`），它不再持有动作选项时回退到
  "扫描全部槽位取第一个带 motion 的"。只用扫描会在"右手拿着手机时点吹泡泡糖"时
  把后者**静默忽略**（右手在清单里排在嘴部之前）；只用记忆会在槽位被换掉后留下陈旧值。
- **姿势不冲突**：`phone*`（5 个）和 `chuipaopao*`（8 个）两组参数本来就不相交。
...
引擎的 `ExpressionManager` **一次只持有一个** `currentExpression` —— 多槽位叠加
（眼镜 + 贴纸 + 情绪同时开）会只剩最后一个。所以参数是插件自己按帧写的
（`expressionLayers`，在 `saveParameters` 缝里、在动作更新之前），
算的是同一种算术：这只模型每个表情都是 `Add`。

顺带**别再试**把它合成成一个表达式交给引擎：试过一次，淡入开始之后就塌掉、什么都不渲染。

**淡入要自己补**：自己按帧写是**瞬时**的，表情会"啪"地出现。现在那一层有 200ms 缓动
（每帧朝目标权重逼近，三种混合模式各自插值；权重到 0 就把条目丢掉、不再写它，
否则"参数永远关不掉"会再演一遍）。

## 头部判定用 drawable 几何，别猜

"点头部才挥锤"靠 `measureHead()`：从模型自己的 drawable 几何算一个**模型空间**的头部包围盒
（所以缩放、拖动之后依然有效），`hitsHead` 再按当前变换映射过去。没有可识别面部 drawable
的模型退化成"点哪都算头"，行为跟以前一致。

## 参数查找别每帧 `Array.from`

`parameterIndex(core, id)` 最直观的写法是 `Array.from(raw.ids).indexOf(id)`，
但那**每次都新建一个数组**。帧内按 id 查上百次（氛围录像一帧 84 次 + 姿势录制 + 表情层）
就是白烧 CPU —— 加一层 `Map<id, index>` 缓存，换模型时清空。
